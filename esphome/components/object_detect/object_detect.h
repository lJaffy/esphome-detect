...
// Forward declaration: the concrete detector (HumanFaceDetect,
// PedestrianDetect, ...) is created in the .cpp via the factory, so the
// header never needs the IDF model headers.
namespace dl {
namespace detect {
class Detect;
}  // namespace detect
...
class ObjectDetect : public Component, public camera::CameraListener {
 public:
  ~ObjectDetect();
  void setup() override;
  void loop() override;
  ...
  void set_object_type(const std::string &type) { this->object_type_ = type; }
  ...
  template<typename F> void add_on_detection_callback(F &&callback) {
    this->detection_callback_.add(std::forward<F>(callback));
  }
 ...
 protected:
  ...
  dl::detect::Detect *create_detector_();
  // No-throw PSRAM buffer reused across frames; avoids per-frame DRAM
  // allocation (std::vector throws -> IDF abort on large frames).
  bool ensure_conv_buf_(size_t size);
  void publish_no_detection_();
  void publish_detection_(const ObjectBox &box, int count);

  camera::Camera *camera_{nullptr};
  std::string object_type_{"face"};
  std::string model_name_{"face/msrmnp"};
  ...
  CallbackManager<void(float, float, float, float, float, int)> detection_callback_;
  ...
  dl::detect::Detect *detector_{nullptr};
  uint8_t *conv_buf_{nullptr};
  size_t conv_buf_size_{0};
};

class ObjectDetectedTrigger : public Trigger<float, float, float, float, float, int> {
 public:
  explicit ObjectDetectedTrigger(ObjectDetect *parent) {
    parent->add_on_detection_callback(
